...
if( $member->is_logged_in() ){ header('Location: memberpage.php'); } 

$stmt = $db->prepare('SELECT resetToken, resetComplete FROM user WHERE resetToken = :token');
...
if(isset($_POST['submit'])){
	...
	if(!isset($error)){
		...
		try {
			...
			$stmt = $db->prepare("UPDATE user SET password = :hashedpassword, resetComplete = 'Yes'  WHERE resetToken = :token");
			$stmt->execute(array(
	    ...
	    <div class="col-xs-12 col-sm-8 col-md-6 col-sm-offset-2 col-md-offset-3">
	    	...
	    	<?php if(isset($stop)){
				...
				<form role="form" method="post" action="" autocomplete="off">
					...
					<hr>
					<div class="row">
						<div class="col-xs-6 col-md-6"><input type="submit" name="submit" value="Change Password" class="button expand" tabindex="3"></div>
					</div>
				</form>
